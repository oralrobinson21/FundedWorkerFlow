...
interface AppContextType {
  ...
  updateUserProfileFull: (name: string, phone: string, defaultZipCode: string) => Promise<void>;
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const updateUserProfileFull = async (name: string, phone: string, defaultZipCode: string) => {
    if (!user) throw new Error("User not logged in");
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/users/${user.id}`, {
        ...
        body: JSON.stringify({ name, phone, defaultZipCode }),
      });
      ...
      const data = await response.json();
      const updatedUser = { ...user, name: data.name, phone: data.phone, defaultZipCode: data.defaultZipCode };
      setUser(updatedUser);
      await saveUser(updatedUser);
    } catch (error) {
      console.error("updateUserProfileFull error:", error);
      throw error;
    }
  };
